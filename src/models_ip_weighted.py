...
def model_ip_weighted(prob, weight_method, instability, minimax, allsol):
	...
	v = m.addVar(lb=-2 ** 8 * len(list(prob.std_type.keys())), ub=len(list(prob.std_type.keys())) * max_rank, vtype=GRB.CONTINUOUS, obj=1.0, name='v')
	...
	############################################################
	if Bio2014:
		b = {}  # # is team t of project p a biology group?
		for p in cal_P:
			for t in range(len(prob.projects[p])):
				b[p, t] = m.addVar(lb=0.0, ub=1.0,
								vtype=GRB.BINARY,
								obj=0.0,
								name='b_%s_%s' % (p, t))
	else:
	...
	if restrictions2016:
		...
		m.addConstr(quicksum(y['04', t] for t in range(len(prob.projects['04']))) <= 0, 'prj4_%s' % g)
	...
	for p in cal_P:
		for t in range(len(prob.projects[p]) - 1):
			m.addConstr(quicksum(x[g, p, t] for g in list(prob.groups.keys())) >= quicksum(x[g, p, t + 1] for g in list(prob.groups.keys())) , "symbreak_%s" % (p))

    ############################################################
    # weighted
	m.addConstr(v >= quicksum(weights[grp_ranks[g][p]] * a[g] * x[g, p, t] for g in list(prob.groups.keys()) for p in list(grp_ranks[g].keys()) for t in range(len(prob.projects[p]))), 'v')
