...
def model_ip(prob):
	...
	for g in list(prob.groups.keys()):
		...
		valid_prjs=[x for x in list(prob.projects.keys()) if prob.projects[x][0][2] in prob.valid_prjtype[peek]]
...
def model_lex(prob, v, h, z, minimax, instability, direction):
	...
	if direction=="bootomup":
		...
		while l>h:
			#print z,v,l
			m.addConstr(z[v-l]>=quicksum(a[g] * x[g,p,t] for (g,p) in grp_x_ranks[l] for t in range(len(prob.projects[p]))),'z_%s' % l)
			l-=1
		m.addConstr(z_h>=quicksum(a[g] * x[g,p,t] for (g,p) in grp_x_ranks[l] for t in range(len(prob.projects[p]))) ,'z_%s' % h)
	elif direction=="topdown":
		l=1
		while l<h:
			#print z,v,l
			m.addConstr(z[l-1] <= quicksum(a[g] * x[g,p,t] for (g,p) in grp_x_ranks[l] for t in range(len(prob.projects[p]))),'z_%s' % l)
			l+=1
		m.addConstr(z_h<=quicksum(a[g] * x[g,p,t] for (g,p) in grp_x_ranks[l] for t in range(len(prob.projects[p]))) ,'z_%s' % h)
	else:
			...
			for t in range(len(prob.projects[p])):
				for g in list(prob.groups.keys()):
					if a[g]<=prob.projects[p][t][1]:
						m.addConstr(slack[p,t]+1-a[g]<= prob.projects[p][t][1]*z2[g,p,t], 'c30_%s_%s_%s' % (g,p,t))
						m.addConstr(a[g]+1-(1-y[p,t])*prob.projects[p][t][0] <= prob.projects[p][t][1]*z2[g,p,t]+(prob.projects[p][t][1]+1)*y[p,t], 'c31_%s_%s_%s' % (g,p,t))
					else:
					...
					if (grp_ranks[g][p2] < grp_ranks[g][p]):
						for t in range(len(prob.projects[p])):
							for t2 in range(len(prob.projects[p2])):
								m.addConstr(q[g,p,t] >= (grp_ranks[g][p] - grp_ranks[g][p2]) * (x[g,p,t] + z2[g,p2,t2] - 1), 'c32_%s_%s_%s' % (g,p,t))
