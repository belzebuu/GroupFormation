...
############################################################
# weighted
	#m.addConstr(v>=quicksum(weights[grp_ranks[g][p]] * a[g] * x[g,p,t] for g in prob.groups.keys() for p in grp_ranks[g].keys() for t in range(len(prob.projects[p]))),  'v')
	...
	m.addConstr(tot_instability >= quicksum(a[g]*q[g,p,t] for g in list(prob.groups.keys()) for p in list(prob.projects.keys()) for t in range(len(prob.projects[p]) ) ), 'instability')
	#m.addConstr(0 == quicksum(q[g,p,t] for g in prob.groups.keys() for p in prob.projects.keys() for t in range(len(prob.projects[p]) ) ), 'instability')
